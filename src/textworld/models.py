...
from utils.layers import ParallelLinear
...
class ParallelGraphAttentionLayer(nn.Module):
    def __init__(
        self, n_abstractions, in_features, out_features, dropout, alpha, concat=True
    ):
        super().__init__()
        ...
        self.W = nn.Parameter(
            torch.zeros(size=(n_abstractions, in_features, out_features))
        )
        nn.init.xavier_uniform_(self.W.data, gain=1.414)
        self.a = nn.Parameter(torch.zeros(size=(n_abstractions, 2 * out_features, 1)))
    ...
    def forward(self, input, adj):
        ...
        a_input = torch.cat(
            [
                h.repeat(1, 1, 1, N).view(batch_size, self.n_abstractions, N * N, -1),
                h.repeat(1, 1, N, 1),
            ],
            dim=2,
        ).view(batch_size, self.n_abstractions, N, -1, 2 * self.out_features)

        e = self.leakyrelu(
            torch.matmul(a_input, self.a.unsqueeze(0).unsqueeze(2)).squeeze(-1)
        )
...
class GraphAttentionLayer(nn.Module):
    ...
    def forward(self, input, adj):
        ...
        batch_size = h.size(0)
        a_input = torch.cat(
            [h.repeat(1, 1, N).view(batch_size, N * N, -1), h.repeat(1, N, 1)], dim=2
        ).view(batch_size, N, -1, 2 * self.out_features)
...
class ParallelGAT(nn.Module):
    def __init__(self, n_abstractions, nfeat, nhid, dropout, alpha, nheads):
        super().__init__()
        self.dropout = dropout

        self.attentions = [
            ParallelGraphAttentionLayer(
                n_abstractions, nfeat, nhid, dropout=dropout, alpha=alpha, concat=True
            )
...
class ParallelCQAttention(torch.nn.Module):
    def __init__(self, n_abstractions, block_hidden_dim, dropout=0):
        super().__init__()
        self.dropout = dropout
        w4C = torch.empty(n_abstractions, block_hidden_dim, 1)
        w4Q = torch.empty(n_abstractions, block_hidden_dim, 1)
        w4mlu = torch.empty(1, n_abstractions, 1, block_hidden_dim)
        ...
        bias = torch.empty(n_abstractions, 1)
    ...
    def forward(self, C, Q, Cmask, Qmask):
        ...
        S1 = masked_softmax(S, Qmask, dim=3)
        S2 = masked_softmax(S, Cmask, dim=2)

        A = torch.matmul(S1, Q)
        B = torch.matmul(torch.matmul(S1, S2.transpose(2, 3)), C)
        out = torch.cat([C, A, torch.mul(C, A), torch.mul(C, B)], dim=3)
        return out
    ...
    def trilinear_for_attention(self, C, Q):
        ...
        subres0 = torch.matmul(C, self.w4C).expand([-1, -1, -1, max_q_len])
        subres1 = (
            torch.matmul(Q, self.w4Q)
            .transpose(2, 3)
            .expand([-1, -1, max_context_len, -1])
        )
        subres2 = torch.matmul(C * self.w4mlu, Q.transpose(2, 3))
        res = subres0 + subres1 + subres2
        res += self.bias.unsqueeze(1)
        return res
...
class CQAttention(torch.nn.Module):
    ...
    def trilinear_for_attention(self, C, Q):
        ...
        subres1 = (
            torch.matmul(Q, self.w4Q).transpose(1, 2).expand([-1, max_context_len, -1])
        )
...
class ParallelScaledDotProductAttention(torch.nn.Module):
    ...
    def forward(self, q, k, v, mask):
        ...
        attn = masked_softmax(attn, mask, 3)
...
class ParallelAttention(nn.Module):
    def __init__(
        self, n_abstractions, query_dim, context_dim, attention_type="general"
    ):
        super().__init__()
        ...
        if self.attention_type == "general":
            self.linear_in = ParallelLinear(
                n_abstractions, query_dim, query_dim, bias=False
            )

        if query_dim != context_dim:
            self.linear_proj = ParallelLinear(
                n_abstractions, query_dim, context_dim, bias=False
            )

        self.linear_out = ParallelLinear(
            n_abstractions, context_dim * 2, context_dim, bias=False
        )
    ...
    def forward(self, query, context):
        ...
        """
        batch_size, n_abstractions, output_len, query_dim = query.size()
        batch_size, _, query_len, context_dim = context.size()

        if self.attention_type == "general":
            query = self.linear_in(query.transpose(1, 0))
            query = query.transpose(1, 0)
        if query_dim != context_dim:
            query = self.linear_proj(query.transpose(1, 0))  # batch x cmds x hidden
            query = query.transpose(1, 0)

        # (batch_size, output_len, dimensions) * (batch_size, query_len, dimensions) ->
        # (batch_size, output_len, query_len)
        attention_scores = torch.matmul(query, context.transpose(2, 3).contiguous())
        ...
        mix = torch.matmul(attention_weights, context)  # batch x cmds x hidden
        ...
        output = self.linear_out(combined.transpose(1, 0)).transpose(
            1, 0
        )  # batch x cmds x hidden
        output = self.tanh(output)
        attention_weights = attention_weights.mean(
            dim=2
        )  # averaged attentions from each command
...
class Attention(nn.Module):
    ...
    **Thank you** to IBM for their initial implementation of :class:`Attention`. Here is
    ...
    def forward(self, query, context):
        ...
        if self.attention_type == "general":
            query = query.reshape(
                batch_size * output_len, query_dim
...
class ParallelSelfAttention(torch.nn.Module):
    ...
    def __init__(
        self, n_abstractions, query_hidden_dim, block_hidden_dim, n_head, dropout=0.1
    ):
        super().__init__()
        ...
        self.w_qs = torch.nn.Parameter(
            torch.zeros(n_abstractions, block_hidden_dim, n_head * block_hidden_dim)
        )
        self.w_ks = torch.nn.Parameter(
            torch.zeros(n_abstractions, block_hidden_dim, n_head * block_hidden_dim)
        )
        self.w_vs = torch.nn.Parameter(
            torch.zeros(n_abstractions, block_hidden_dim, n_head * block_hidden_dim)
        )

        torch.nn.init.normal_(
            self.w_qs, mean=0, std=np.sqrt(2.0 / (block_hidden_dim * 2))
        )
        torch.nn.init.normal_(
            self.w_ks, mean=0, std=np.sqrt(2.0 / (block_hidden_dim * 2))
        )
        torch.nn.init.normal_(
            self.w_vs, mean=0, std=np.sqrt(2.0 / (block_hidden_dim * 2))
        )

        ####
        self.attention = ParallelScaledDotProductAttention(
            temperature=np.power(block_hidden_dim, 0.5), dropout=dropout
        )
    ...
    def forward(self, q, mask, k, v):
        ...
        batch_size, len_q = q.size(0), q.size(2)
        len_k, len_v = k.size(2), v.size(2)
        assert mask.size(2) == len_q
        assert mask.size(3) == len_k

        # q = self.w_map(q) # map to the key/value space
        residual = q

        q = torch.matmul(q, self.w_qs).view(
            batch_size, self.n_abstractions, len_q, self.n_head, self.block_hidden_dim
        )
        k = torch.matmul(k, self.w_ks).view(
            batch_size, self.n_abstractions, len_k, self.n_head, self.block_hidden_dim
        )
        v = torch.matmul(v, self.w_vs).view(
            batch_size, self.n_abstractions, len_v, self.n_head, self.block_hidden_dim
        )

        q = (
            q.permute(3, 0, 1, 2, 4)
            .contiguous()
            .view(-1, self.n_abstractions, len_q, self.block_hidden_dim)
        )  # (n*b) x A x lq x dk
        k = (
            k.permute(3, 0, 1, 2, 4)
            .contiguous()
            .view(-1, self.n_abstractions, len_k, self.block_hidden_dim)
        )  # (n*b) x A x lk x dk
        v = (
            v.permute(3, 0, 1, 2, 4)
            .contiguous()
            .view(-1, self.n_abstractions, len_v, self.block_hidden_dim)
        )  # (n*b) x A x lv x dv

        mask = mask.repeat(self.n_head, 1, 1, 1)  # (n*b) x .. x ..
        output, attn = self.attention(q, k, v, mask=mask)
        output = output.view(
            self.n_head, batch_size, self.n_abstractions, len_q, self.block_hidden_dim
        )
        output = (
            output.permute(1, 2, 3, 0, 4)
            .contiguous()
            .view(batch_size, self.n_abstractions, len_q, -1)
        )  # b x lq x (n*dv)
...
class SelfAttention(torch.nn.Module):
    ...
    def __init__(self, query_hidden_dim, block_hidden_dim, n_head, dropout=0.1):
        ...
        torch.nn.init.normal_(
            self.w_qs.weight, mean=0, std=np.sqrt(2.0 / (block_hidden_dim * 2))
        )
        torch.nn.init.normal_(
            self.w_ks.weight, mean=0, std=np.sqrt(2.0 / (block_hidden_dim * 2))
        )
        torch.nn.init.normal_(
            self.w_vs.weight, mean=0, std=np.sqrt(2.0 / (block_hidden_dim * 2))
        )
