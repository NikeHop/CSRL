...
from utils.prior_kg.utils import text_abstraction
...
class Tokenizer:
    ...
    def extract_entity(self,text, vocabulary, class_tree, n_abstractions, noun_only_tokens=None, stopwords = None):
        ...
        text = self.extractor(text, vocabulary, self.ngram, stopwords=stopwords) 
        abstracted_text, mask = text_abstraction(text,
                                                 class_tree,
                                                 n_abstractions) # [sentence [abstractions]]
        return abstracted_text, mask

    def extract_entity_ids(self, text, vocabulary, class_tree, n_abstractions, unk=True):
        words, mask = self.extract_entity(text, vocabulary, class_tree, n_abstractions) # [sentence [abstraction]]
        word_ids = []
        for word in words:
            new_ids = []
            for abstraction in word:
                try:
                    index = self._get_word_id(abstraction, vocabulary, unk)
                    new_ids.append(index)
                except KeyError:
                        pass
            word_ids.append(new_ids)
