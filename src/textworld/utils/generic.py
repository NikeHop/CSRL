...
def load_embeddings(opt, emb_loc, emb_type, separator="[<>+]", limit=None):
    print(emb_loc, emb_type)
    print("Loading " + emb_type + " embeddings ...", end="")
    print(emb_dict)
    if emb_type in emb_dict:
        print(" from cache.")
        return emb_dict[emb_type], emb_dict['hyperbolic']
    if emb_type == "w2v":
        emb_model = KeyedVectors.load_word2vec_format(
            emb_loc + "GoogleNews-vectors-negative300.bin", binary=True, limit=limit
        )
    elif emb_type == "fasttext":
        emb_model = KeyedVectors.load_word2vec_format(
            emb_loc + "crawl-300d-2M.vec", binary=False, limit=limit
        )
    elif emb_type == "glove":
        emb_model = api.load("glove-wiki-gigaword-300")
    elif emb_type == "complex":
        emb_model = KeyedVectors.load_word2vec_format(
            emb_loc + "complex.txt", binary=False, limit=limit
        )
    elif emb_type == "conceptnet-55-ppmi":
        emb_model = KeyedVectors.load_word2vec_format(
            emb_loc + "conceptnet-55-ppmi-en.txt", binary=False, limit=limit
        )
    elif emb_type == "numberbatch":
        emb_model = KeyedVectors.load_word2vec_format(
            emb_loc + "numberbatch-en-19.08.txt", binary=False, limit=limit
        )
        if opt.hyperbolic:
            emb_model_hyperbolic = KeyedVectors.load_word2vec_format(
                emb_loc + "hypernym_noun.100d.txt", binary=False, limit=limit
            )
        else:
            emb_model_hyperbolic = None
    elif emb_type == "transh":
        emb_model = KeyedVectors.load_word2vec_format(
            emb_loc + "transh.txt", binary=False, limit=limit
        )
    elif emb_type == "random":
        emb_model = KeyedVectors(300)
        with open(os.path.join(opt.vocab_directory, "vocab.txt"), "r") as file:
            embeddings = nn.Embedding(num_embeddings=1000, embedding_dim=300)
            for i, line in enumerate(file):
                emb_model[line.strip()] = np.array(embeddings(torch.tensor(i)).detach())
        if opt.hyperbolic:
            emb_model_hyperbolic = KeyedVectors.load_word2vec_format(
                emb_loc + "hypernym_noun.100d.txt", binary=False, limit=limit
            )
        ...
        if len(types) <= 1:
            ...
            return None
        models = [
            load_embeddings(emb_loc, t, separator=separator, limit=limit) for t in types
        ]
    ...
    emb_dict['hyperbolic'] = emb_model_hyperbolic
    print("  (" + str(len(emb_model.vocab)) + " words) Done.")
    return emb_model, emb_model_hyperbolic
...
def align_models(models, padding="random"):
    ...
    return [
        align_vocabulary(m, global_vocab, padding=padding) if m else None
        for m in models
    ]
