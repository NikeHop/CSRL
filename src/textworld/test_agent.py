            ...
            for step_no in range(opt.max_step_per_episode):
                ...
                if agent.graph_emb_type and (
                    ...
                    agent.update_current_graph(
                        obs, commands, scored_commands, infos, opt.graph_mode
                    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--token_extractor", default="max", help="token extractor: (any or max)"
    )
    parser.add_argument(
        "--corenlp_url",
    ...
    # Arguments related to the abstraction
    parser.add_argument(
    ...
    for n in range(opt.start_run,opt.nruns):
        ...
        opt.pretrained_model = opt.results_filename + ".pt"
        ...
        starttime = time()
        print("\n RUN ", n, "\n")
        test_scores, test_norm_scores, test_moves = play(
            myagent, opt, random_action=random_action
        )
