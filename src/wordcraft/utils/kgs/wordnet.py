...
def get_class_hierachies(instances:list)->list:
    ...
    for instance in tqdm.tqdm(instances):
        ...
        hypernyms = [hypernym for hypernym in synset.hypernyms()+synset.instance_hypernyms() if hypernym.pos()=='n']
        if len(hypernyms)==0:
            # No hypernym exist
            if len(instance_components)>1:
                cl_hierachy[instance] = []
            ...
            while len(hypernyms)>0:
                ...
                synset = hypernym
                hypernyms = [h for h in hypernym.hypernyms()+hypernym.instance_hypernyms() if h.pos()=='n']
                i+=1
        class_trees.append(cl_hierachy)
    return class_trees

def collapse_class_tree(class_tree:dict,instances:list,start:int,end:int)->None:
    """
...
def get_class_tree_wordnet(instances:list,collapse:bool,start:int,end:int,visualize=False):
    """
    ...
    if not os.path.exists('./kgs'):
        os.mkdir('./kgs')
    with open('./kgs/wordnet.pkl','wb+') as f:
