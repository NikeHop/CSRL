	...
	def __init__(
		...
		if feature_type == 'one_hot':
			...
			if loaded is not None:
				...
				for e in words:
					tokens_embeddings = []
					# Some class names are compound nouns connected via _
					if '_' in e:			
						for token in e.split('_'):
							v = word2vec(token).vector
							# If the word cannot be found insert random embedding
							if np.sum(v)==0:
								number_random += 1
								b = random_feature_std/np.sqrt(2)
								v = self.np_random.laplace(
												loc=0, scale=b, size=(random_feature_size)
												)
							tokens_embeddings.append(v)
					else:	
