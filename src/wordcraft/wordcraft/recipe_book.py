...
class RecipeBook:
    def __init__(self, 
        ...
        self.entities = list(self._rawdata['entities'].keys())
    ...
    def _random_choice(self, options):
        # Fast random choice
        i = i = self.np_random.randint(0, len(options))
        return options[i]
    ...
    def _init_data_split(self, split, train_ratio):
        ...
        elif split == 'by_ingredients':
            self.test_entities = random.sample(self.entities,int(len(self.entities)*(1-train_ratio)))
            for depth in depths:
                tasks = self.depth2task[depth]
                
                for task in tasks:
                    task_entities = [task.goal] + list(task.base_entities) + list(task.intermediate_entities)
                    is_test_task = False
                    for entity in task_entities:
                        if entity in self.test_entities:
                            self.depth2task_test[depth].append(task)
                            is_test_task = True
                            break
                    if not is_test_task: self.depth2task_train[depth].append(task)
